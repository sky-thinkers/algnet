...
std::shared_ptr<IConnection> ConnectionParser::parse_connection(
    ...
    if (sender_id.empty()) {
        throw std::runtime_error("Sender ID is not specified for connection " + conn_id);
    }

    Id receiver_id = value_node["receiver_id"].as<Id>();
    if (receiver_id.empty()) {
        throw std::runtime_error("Receiver ID is not specified for connection " + conn_id);
    }
    ...
    if (packets == 0) {
        throw std::runtime_error("Number of packets to send is not specified for connection " + conn_id);
    }

    std::string mplb_name = value_node["mplb"].as<std::string>();
    if (mplb_name.empty()) {
        throw std::runtime_error("MPLB algorithm is not specified for connection " + conn_id);
    }
    auto mplb = make_mplb(mplb_name);

    auto conn = std::make_shared<ConnectionImpl>(
        conn_id, sender_ptr, receiver_ptr,
    ...
    if (!flows) {
        throw std::runtime_error("No flows specified for connection " + conn_id);
    }
